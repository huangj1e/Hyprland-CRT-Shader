...
fn append_once(path: &Path, marker: &str, block: &str) -> Result<(), String> {
    ...
    text.push('\n');
    text.push_str(block);
    fs::write(path, text).map_err(|e| format!("无法更新 {}: {e}", path.display()))
}

fn persist_effect(enabled: bool, shader: &Path) -> Result<(), String> {
    let user_dir = config_dir();
    fs::create_dir_all(&user_dir).map_err(|e| e.to_string())?;

    let persistent_conf = user_dir.join("hyprland-crt-shader.conf");
    let conf = if enabled {
        format!(
            "# Generated by hypr-crt-control.\n\
             decoration {{\n    screen_shader = {}\n}}\n\n\
             debug {{\n    damage_tracking = 0\n    vfr = false\n}}\n",
            shader.display()
        )
    ...
    fs::write(&persistent_conf, conf).map_err(|e| e.to_string())?;

    let hypr_dir = base_config_dir().join("hypr");
    fs::create_dir_all(&hypr_dir).map_err(|e| e.to_string())?;
    let main_conf = hypr_dir.join("hyprland.conf");
    let main_lua = hypr_dir.join("hyprland.lua");
    let has_conf = main_conf.exists();
    let has_lua = main_lua.exists();

    if has_conf || !has_lua {
        let marker = "# hypr-crt-control persistent settings";
        let block = format!("{marker}\nsource = {}\n", persistent_conf.display());
        append_once(&main_conf, marker, &block)?;
    }

    if has_lua {
        let persistent_lua = user_dir.join("hyprland-crt-shader.lua");
        let shader = shader
            .display()
            .to_string()
            .replace('\\', "\\\\")
        ...
        let lua = if enabled {
            ...
            "-- Generated by hypr-crt-control.\n\
             ...
             debug = { damage_tracking = 1, vfr = true } })\n"
                .into()
        };
        fs::write(&persistent_lua, lua).map_err(|e| e.to_string())?;
        let marker = "-- hypr-crt-control persistent settings";
        let lua_path = persistent_lua
            .display()
            .to_string()
            .replace('\\', "\\\\")
            .replace('"', "\\\"");
        let block = format!("{marker}\nrequire(\"{lua_path}\")\n");
        append_once(&main_lua, marker, &block)?;
    }
    Ok(())
}
...
fn apply(window: &AppWindow, user_shader: &Path) -> Result<(), String> {
    ...
    let enabled = window.get_effect_enabled();
    persist_effect(enabled, user_shader)?;
    if enabled {
